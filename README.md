...
## 当日の細かい注意
- **ロゴ画像**：`taitoru.jpg`（原本・Dropbox 側にのみ置く＝リポジトリには入れない）から `client/public/img/` の hero／logo を切り出している。作り直すときは同じ比率で切って WebP（品質 80）＋JPEG を出し、`scripts/assets_test.js` を通すこと。
